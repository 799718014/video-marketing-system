        ...
        <section className="card storyboard-card">
          ...
          <div className="scene-list">{draftScenes.map((scene, index) => <article className="scene" key={index}>
            ...
            <div className="form-grid compact">
              ...
              <label className="wide">运动提示词<textarea value={scene.motion_prompt} onChange={(event) => updateScene(index, { motion_prompt: event.target.value })} placeholder={scene.generation_strategy === 'text_to_video' ? '文生视频使用场景提示词，此处留空即可' : '镜头缓慢推进…'} disabled={scene.generation_strategy === 'text_to_video'} /></label>
              {scene.generation_strategy === 'text_to_video' && <label className="wide">使用场景提示词<textarea value={scene.scene_prompt ?? ''} onChange={(event) => updateScene(index, { scene_prompt: event.target.value })} placeholder="年轻女性坐在明亮化妆台前的镜子前，使用参考图中的心形耳环。她将耳环戴到左耳，轻轻整理头发并看向镜中微笑。镜头由侧脸中景缓慢推进至耳部特写，晨间柔和自然光。保持参考商品的形状、材质、颜色、尺寸比例和佩戴位置不变；不得生成文字、价格或品牌 Logo。" /></label>}
            </div>
